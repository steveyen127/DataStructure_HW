...
int main(){
	...
	if(wf == NULL){
		...
		for(filenumber = 3; filenumber<=3; filenumber++){
			totalCost = 0;
			root = NULL;
...
void vertexCover(struct node* root){
	...
	if(root->data == 0){
		...
		//if root is already covered by other base.
			...
			if(root->left!= NULL || root->right!= NULL){
				...
				else if(costSolo > costChildL && costChildL < costChildR){
					totalCost = totalCost + costChildL;
					root->left->data = -1002;
				...
				else if(costSolo > costChildR && costChildL > costChildR){
					totalCost = totalCost + costChildR;
					root->right->data = -1002;
